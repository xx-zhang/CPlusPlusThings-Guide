...
> **上次截止**：2026-09-21（CG 导读 `31` 完成、上游本地修订提交到独立分支后）。
...
（本次）`29` 具身智能技术栈覆盖审计：克隆 9 个行业仓库（rclcpp/ros2_control/eigen/gtsam/pinocchio/mujoco/ompl/taskflow/pybind11）扫描，产出覆盖矩阵与缺口清单
（本次）`30` 原仓库阅读导览：把上游 CPlusPlusThings 的读法降负担（先读什么/跳过什么/哪些示例是故意编译失败）
（本次）`31` **CppCoreGuidelines 中文导读**：克隆官方仓库（460 条条目 = 441 个唯一编号/30 主节），产出「结构全图 + 441 编号全索引中译 + P 理念 13 条全译 + 对账（18 互证/4 张力/10 缺口/7 处我们更强）+ 阅读规划」；**自查抓到自己的两处错**（"460 条"应精确为 441 编号；漏 7 条 Per.18/19/30 + T.142/143/144/150 已补齐重验 441/441）
（本次）**上游本地修订落地**：`~/CPlusPlusThings` 的 7 处阅读修订（含新建 `func_pointer/README.md`、`auto_ptr`→`my_auto_ptr` 保住教学内容的决策）提交到**独立分支 `local-reading-fixes`**（`138458e`），master 保持干净以便 `git pull`；同一修订另存 `local-reading-fixes.patch` 于本仓库（双备份）
```
...
| 上游克隆的本地修订放**独立分支** + 存成补丁 | 让 master 能随时 `git pull` 上游而不冲突；补丁是第二份保险 |
| CG 只做「导读」不做全译 | CG 是活文档（译文必过期）且社区已有全译；价值在**结构地图 + 441 编号索引 + 对账 + 阅读规划** |
